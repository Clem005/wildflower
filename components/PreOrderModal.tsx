...
import { sendPreOrder } from "@/app/actions/sendPreOrder";
...
export default function PreOrderModal({ isOpen, onClose, product }: PreOrderModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await sendPreOrder(formData);
            ...
            {status === "success" ? (
              <div className="text-center py-10">
                ...
                <p className="font-sans text-sm text-charcoal/70 mb-8">
                  Thank you for pre-ordering the {product.name}. We&apos;ve sent a confirmation to your email and will be in touch soon!
                </p>
                <button
                  onClick={onClose}
              ...
              <>
                <div className="mb-8">
                  ...
                  <p className="font-sans text-sm text-charcoal/60 mt-1">
                    {product.buyNowSection.price} {product.buyNowSection.unit}
                  </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  >
                    {status === "loading" ? "Processing..." : "Complete Pre-Order"}
                  </button>
                </form>
              </>
            )}
